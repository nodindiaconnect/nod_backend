...
function parseStringArrayField(value, { fieldName, required = false, maxItems = MAX_ARRAY_ITEMS, maxItemLength = MAX_TEXT_LENGTH, mustBeUrl = false }) {
    if (value === undefined || value === null) {
        if (required) return { ok: false, message: `${fieldName} is required.` };
        return { ok: true, value: [] };
    }
    if (!Array.isArray(value)) return { ok: false, message: `${fieldName} must be an array.` };
    if (required && value.length === 0) return { ok: false, message: `${fieldName} is required.` };
    if (value.length > maxItems) return { ok: false, message: `${fieldName} can have at most ${maxItems} items.` };

    const cleaned = [];
    for (const item of value) {
        if (typeof item !== "string") return { ok: false, message: `${fieldName} items must be strings.` };
        ...
        if (mustBeUrl && !isValidUrl(trimmed)) return { ok: false, message: `${fieldName} must contain valid URLs.` };
    ...
    static async getMyProfile(req, res) {
        try {
            ...
            return helper.success(res, "Profile fetched successfully", {
                ...
                yearsOfExperience: designer.yearsOfExperience ?? profileBlob.experience ?? "",
                ...
                photos: safeArray(designer.photos),
    ...
    static async editMyProfile(req, res) {
        try {
            ...
            const arrayValidators = {
                ...
                designStyles: { value: designStyles, fieldName: "designStyles", required: true },
                specializations: { value: specializations, fieldName: "specializations", required: true },
                serviceCities: { value: serviceCities, fieldName: "serviceCities", required: true },
                photos: { value: photos, fieldName: "photos", mustBeUrl: true, maxItems: 30 },
                portfolioLinks: { value: portfolioLinks, fieldName: "portfolioLinks", mustBeUrl: true },
            };
            ...
            // --------------------------------------------------
            if (yearsOfExperience !== undefined && (typeof yearsOfExperience !== "number" || isNaN(yearsOfExperience) || yearsOfExperience < 0)) {
                return helper.failed(res, "yearsOfExperience must be a valid positive number");
            }
            if (minBudgetHandled !== undefined && (typeof minBudgetHandled !== "number" || isNaN(minBudgetHandled) || minBudgetHandled < 0)) {
                return helper.failed(res, "minBudgetHandled must be a valid positive number");
            }
            if (maxBudgetHandled !== undefined && (typeof maxBudgetHandled !== "number" || isNaN(maxBudgetHandled) || maxBudgetHandled < 0)) {
                return helper.failed(res, "maxBudgetHandled must be a valid positive number");
            }
            if (
                ...
                return helper.failed(res, "Minimum budget cannot be greater than maximum budget");
            }
            ...
            const normalizedExperienceLevel =
                experienceLevel !== undefined ? String(experienceLevel).trim().toUpperCase() : undefined;
            const normalizedAvailability =
                availability !== undefined ? String(availability).trim().toUpperCase() : undefined;

            if (normalizedExperienceLevel !== undefined && !EXPERIENCE_LEVELS.includes(normalizedExperienceLevel)) {
                return helper.failed(res, `experienceLevel must be one of: ${EXPERIENCE_LEVELS.join(", ")}`);
            }
            if (normalizedAvailability !== undefined && !AVAILABILITY_STATUSES.includes(normalizedAvailability)) {
                return helper.failed(res, `availability must be one of: ${AVAILABILITY_STATUSES.join(", ")}`);
            }

            const sanitizedBio = bio !== undefined ? sanitizeData(String(bio).trim()).slice(0, 1000) : undefined;

            // --------------------------------------------------
            // Update User + Designer together
            // --------------------------------------------------
            const result = await prisma.$transaction(async (tx) => {
                // Preserve the profile JSON blob shape, only patch bio into it.
                // This keeps legacy fields (specialization/style/experience) intact
                // for any other code still reading `user.profile` directly.
                const existingProfileBlob = parseProfileBlob(user.profile);
                const nextProfileBlob =
                    sanitizedBio !== undefined
                        ? { ...existingProfileBlob, bio: sanitizedBio }
                        : existingProfileBlob;
                ...
                const updatedUser = await tx.user.update({
                    ...
                    data: {
                        ...
                        ...(sanitizedBio !== undefined && {
                            profile: JSON.stringify(nextProfileBlob),
                        }),
                    },
                    select: {
                        id: true,
                ...
                const designerData = {
                    ...
                    ...(normalizedExperienceLevel !== undefined && { experienceLevel: normalizedExperienceLevel }),
                    ...(normalizedAvailability !== undefined && { availability: normalizedAvailability }),
                    ...(cleanedArrays.certifications !== undefined && { certifications: cleanedArrays.certifications }),
                    ...(cleanedArrays.designStyles !== undefined && { designStyles: cleanedArrays.designStyles }),
                    ...(maxBudgetHandled !== undefined && { maxBudgetHandled }),
                    ...(minBudgetHandled !== undefined && { minBudgetHandled }),
                    ...(cleanedArrays.photos !== undefined && { photos: cleanedArrays.photos }),
                    ...(cleanedArrays.portfolioLinks !== undefined && { portfolioLinks: cleanedArrays.portfolioLinks }),
                    ...(cleanedArrays.serviceCities !== undefined && { serviceCities: cleanedArrays.serviceCities }),
                    ...(cleanedArrays.specializations !== undefined && { specializations: cleanedArrays.specializations }),
                };
